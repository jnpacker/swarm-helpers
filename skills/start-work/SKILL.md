...
Call `mcp__jira-mcp-server__get_issue` on the parent ticket ID. Record its summary, components, target version, labels, and Activity Type (work_type).

## Step 3 — Gather work summary

Ask the user for a 1–2 sentence description of the work to be done. This becomes the sub-task title. If no issue type was provided as an arg, ask for that too (default: `Sub-task`).

## Step 4 — Create the Jira sub-task

Call `mcp__jira-mcp-server__create_issue` with:
...
Use `mcp__jira-mcp-server__add_comment` to post:
...
## Step 7 — Format the Jira description
...
### MCP tools
- `mcp__jira-mcp-server__get_issue` — fetch parent ticket details
- `mcp__jira-mcp-server__create_issue` — create the sub-task
- `mcp__jira-mcp-server__add_comment` — post context comment on new ticket

### Related skills
- `task-specialist` — formats Sub-task/Task descriptions
- `bug-specialist` — formats Bug descriptions
- `spike-specialist` — formats Spike descriptions
- `finish-work` — closes out the work session started here
