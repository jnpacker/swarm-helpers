...
Use the registered Jira MCP tools to search for and manage spikes. For new spike creation, delegate to `jira-create`. JQL queries filtered by `issuetype = Spike` combined with status, priority, or resolved date are the primary way to find and track research work.

**Spike Content Best Practices** (use when drafting content via `jira-create`):
...
- `template.md` — Authoritative section structure for spike descriptions.
